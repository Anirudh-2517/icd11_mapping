...
function Navbar() {
  ...
  const navItems = [
    ...
    { name: "Products", path: "/products", icon: Package, dropdown: true },
  ...
  const NavLink = ({ to, children, icon: Icon, dropdown }) => {
    ...
    return (
      <div className="relative group">
        <Link
          to={to}
          className={`relative px-3 py-2 text-base transition-all duration-300 flex items-center gap-2 ${
      ...
      >
        <div className="w-full px-4 sm:px-6">
          <div className="flex justify-between items-center py-6">
            <Link
              to="/"
              className="flex items-center gap-3 bg-white/95 backdrop-blur-sm px-4 py-3 rounded-lg shadow-lg border border-white/20 transform transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5"
            >
              <img src="/images/logo.jpeg" alt="Numa Agri Prime Logo" className="w-14 h-14 object-contain" />
              <h1 className="text-gray-800 text-xl sm:text-2xl font-bold tracking-wide">NUMA AGRI PRIME</h1>
            </Link>
            <div className="hidden md:flex items-center space-x-4 ml-auto">
              {navItems.map((item) => (
                <NavLink key={item.name} to={item.path} icon={item.icon} dropdown={item.dropdown}>
                  {item.name}
                </NavLink>
              ))}
            </div>
            <button
              className="md:hidden p-2.5 rounded-xl text-white hover:bg-white/10 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-white/20 ml-auto"
